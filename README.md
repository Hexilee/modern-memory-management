...
> Rust 中变量和引用默认都是不可变的，需要用 `mut` 限定词来使其可变，这与 Cpp 刚好相反。

Rust 的引用同样也分可变引用和不可变引用，Cpp 中对可变引用的约束规则 Rust 也全部涵盖了；并且我们可以注意到，不同于 Cpp 中作为变量别名的引用，Rust 中的引用更像是指针，很多场景下都需要显式地取引用（`&`）和解引用（`*`）。

> Rust 中不能直接拿到裸指针。

看完这个例子后我们继续刚才话题， 第一个问题，Rust 的引用可能比被引用变量本身活得更长吗？答案是不能（在不使用 Unsafe Rust 的情况下）。只要程序过了编译，Rust 能永远保证引用有效。
...
那么，这个约束对线程安全有什么帮助呢？**一个可变引用不能与其他引用同时存在**，再加上后面会提到的**变量在被引用时不能移动**，这就意味着在理想情况下是绝对不会出现数据竞争的。
...
你会发现这个叫作 `Drop`，看起来像是析构函数的 `trait` 跟 Cpp 析构函数还是有明显差别的：`Drop::drop` 只会在所有权被 drop 的时候被调用（拥有所有权的变量被 drop）。
